...
# This patch is injected into the live application scope. Previous late patches
# were outside the original IIFE and therefore could not reach its local state.

a=s.find('function renderLeader(g){', s.find('function leaderPts(g){'))
b=s.find('function drawLeaderHandles(g){', a)
if a<0 or b<0: raise SystemExit('leader render block not found')
render=r'''function kbLeaderTextPointV17(g){
 const {p2,p3}=leaderPts(g),fs=+g.dataset.fontSize||4,tx=parseFloat(g.dataset.textX),ty=parseFloat(g.dataset.textY);
 return {x:Number.isFinite(tx)?tx:(p2.x+p3.x)/2,y:Number.isFinite(ty)?ty:p2.y-fs*.85};
}
function renderLeader(g){
 ...
 const len=Math.hypot(p2.x-p1.x,p2.y-p1.y)||1,ux=(p2.x-p1.x)/len,uy=(p2.y-p1.y)/len;drawArrow(g,p1,ux,uy,as,aa,c,lw,1,sty);
...
if a<0 or b<0: raise SystemExit('leader handles block not found')
...
if close_i<0 or a<0 or b<0: raise SystemExit('leader pointer pipeline not found')
pipeline=r'''paper.addEventListener('pointerdown',e=>{
 const p=pt(e),obj=groupType(e.target),h=e.target.closest?.('.leader-handle');
 if(h){
   const id=h.dataset.owner,g=paper.querySelector(`[data-type="leader"][data-id="${id}"]`);if(!g)return;
   const kind=h.dataset.leaderHandle,tp=kbLeaderTextPointV17(g);selected=g;lastEditable=g;
 ...
 if(tool==='leader'){
   e.preventDefault();e.stopImmediatePropagation();leaderDraft={p1:p,p2:p};
 ...
 if(obj?.dataset.type==='leader'){
   const tp=kbLeaderTextPointV17(obj);selected=obj;lastEditable=obj;drawSelection();
   leaderDrag={obj,start:p,kind:'move',p1:parsePt(obj.dataset.p1),p2:parsePt(obj.dataset.p2),textX:tp.x,textY:tp.y};e.preventDefault();e.stopImmediatePropagation();
 }
},true);
paper.addEventListener('pointermove',e=>{
 ...
 if(d.kind==='move'){const dx=p.x-d.start.x,dy=p.y-d.start.y;g.dataset.p1=`${d.p1.x+dx},${d.p1.y+dy}`;g.dataset.p2=`${d.p2.x+dx},${d.p2.y+dy}`;g.dataset.textX=String(d.textX+dx);g.dataset.textY=String(d.textY+dy)}
 else if(d.kind==='p1')g.dataset.p1=`${p.x},${p.y}`;
 else if(d.kind==='p2'){const dx=p.x-d.p2.x,dy=p.y-d.p2.y;g.dataset.p2=`${p.x},${p.y}`;g.dataset.textX=String(d.textX+dx);g.dataset.textY=String(d.textY+dy)}
...
window.addEventListener('pointerup',e=>{
 if(leaderDraft&&tool==='leader'){
   const p2=pt(e),p1=leaderDraft.p1;leaderDraft=null;leaderPreview?.remove();leaderPreview=null;if(Math.hypot(p2.x-p1.x,p2.y-p1.y)>2)createLeader(p1,p2);
 }
 leaderDrag=null;
},true);
'''
...
if pos<0: raise SystemExit('leader dblclick anchor not found')
pos+=len(dbl)
popup=r'''
...
window.addEventListener('pointerdown',e=>{
 const header=e.target.closest?.('#leaderPopupHeader');if(!header||e.target.id==='leaderPopupClose')return;
 const q=$('leaderPopup'),r=q.getBoundingClientRect();kbLeaderPopupDragV17={id:e.pointerId,dx:e.clientX-r.left,dy:e.clientY-r.top};
 try{header.setPointerCapture(e.pointerId)}catch{}e.preventDefault();e.stopImmediatePropagation();
},true);
window.addEventListener('pointermove',e=>{
 const d=kbLeaderPopupDragV17;if(!d||d.id!==e.pointerId)return;const q=$('leaderPopup');
...
window.addEventListener('pointerup',e=>{if(kbLeaderPopupDragV17&&kbLeaderPopupDragV17.id===e.pointerId){kbLeaderPopupDragV17=null;e.preventDefault();e.stopImmediatePropagation()}},true);
window.addEventListener('pointercancel',e=>{if(kbLeaderPopupDragV17&&kbLeaderPopupDragV17.id===e.pointerId)kbLeaderPopupDragV17=null},true);
'''
s=s[:pos]+popup+s[pos:]

for token in ['KB911_V17_LEADER_CORE_IN_SCOPE','kbLeaderTextPointV17','data-leader-handle\':\'text','kbLeaderPopupDragV17']:
    if token not in s: raise SystemExit('v17 guard missing: '+token)
