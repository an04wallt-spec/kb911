...
if errorlevel 1 exit /b 1
python patch_project_icon_v32.py
if errorlevel 1 exit /b 1
python patch_dimension_label_side.py
if errorlevel 1 exit /b 1
python patch_undo_history_v27.py
if errorlevel 1 exit /b 1
python patch_donate_link_v28.py
if errorlevel 1 exit /b 1
python patch_donate_button_v29.py
if errorlevel 1 exit /b 1
python patch_autosave_v30.py
if errorlevel 1 exit /b 1
python patch_native_autosave_v30.py
if errorlevel 1 exit /b 1
python patch_native_window_guard_v31.py
if errorlevel 1 exit /b 1
python patch_tool_modes_v31.py
if errorlevel 1 exit /b 1
python patch_dimension_input_v32.py
if errorlevel 1 exit /b 1
python patch_active_tool_outline_v33.py
if errorlevel 1 exit /b 1
if not exist build mkdir build
python -c "from pathlib import Path; import re; s=Path(r'app\KB911.html').read_text(encoding='utf-8'); assert 'KB911_V10_CORE_INTERACTIONS' in s; assert 'KB911_V12_EDITING_CORE' in s; assert 'KB911_V17_LEADER_CORE_IN_SCOPE' in s; assert 'KB911_V18_LEADER_TEXT_LOCK_AND_MENU_CLOSE' in s; assert 'KB911_V19_LAYER_MENU_SINGLE_OWNER' in s; assert 'KB911_V20_PRINT_RESTORE' not in s; assert 'KB911_V21_IMAGE_MENU_COMPAT' in s; assert 'KB911_V22_LINE_TOOL_AND_REDO' in s; assert 'KB911_V23_FREE_THREE_POINT_LINE' in s; assert 'KB911_V24_LINE_COPY_PASTE' in s; assert 'KB911_V25_LINE_MIDPOINT_DELETE_AND_HANDLE_SIZE' in s; assert 'KB911_V26_BRAND_EMAIL_COPY' in s; assert 'KB911_V30_AUTOSAVE_RECOVERY' in s; assert 'KB911_V31_STICKY_CREATION_TOOLS' in s; assert 'KB911_V32_DIMENSION_INPUT_PREVIEW' in s; assert 'KB911_V33_ACTIVE_TOOL_OUTLINE' in s; assert 'KB911_V11_EDITING_REPAIR' not in s; assert 'KB911_V13_LEADER_EXPORT_REPAIR' not in s; assert 'KB911_V15_LEADER_CORE_FIX' not in s; assert 'KB911_V16_LEADER_NATIVE_CORE' not in s; assert \"if(tool==='dimension')\" in s; assert \"if(activeImage!==obj){drag=null;return}\" in s; assert \"['dimText','value']\" in s; assert \"['fontSize','fontSize']\" in s; assert \"['textSize','fontSize']\" in s; assert 'editText(obj)' in s; assert 'kbLeaderTextPointV17' in s; assert \"data-leader-handle':'text\" not in s; assert 'kbLeaderPopupDragV17' in s; assert 'kbInstallLayerMenuV19' in s; assert \"setProperty('display','none','important')\" in s; assert 'id=\\\"printSheet\\\"' not in s; assert 'function kbPrintCurrentSheetV20()' not in s; assert 'window.print()' not in s; assert 'function closeImageMenu()' in s; assert 'function openImageMenu(img,e)' in s; assert 'data-tool=\\\"line\\\"' in s; assert 'id=\\\"linePopup\\\"' in s; assert 'function renderSimpleLine(' in s; assert 'function drawSimpleLineHandles(' in s; assert \"data-p3':`${p3.x},${p3.y}`\" in s; assert 'simpleLineDraft={stage:1,p1:p,p2:null}' in s; assert 'kbLineClipboardV24' in s; assert 'kbStartLinePasteV24' in s; assert 'kbPlaceLinePasteV24' in s; assert \"r:.72,class:'handle selection-ui leader-handle'\" in s; assert \"r:.72,class:'handle selection-ui simple-line-handle'\" in s; assert \"selected.dataset.straight='1'\" in s; assert \"simpleLineSelectedHandle==='p2'\" in s; assert 'id=\\\"redoImage\\\"' in s; assert 'id=\\\"kbBrand\\\"' in s; assert 'id=\\\"copySupportEmail\\\"' in s; assert 'schkaf@yandex.ru' in s; assert 'if(!kbHistoryPointer)return;' in s; assert 'if(Math.abs(signed)<=dead)return{down:0,up:0}' in s; assert 'button[data-tool=\\\"dimension\\\"].active' in s; assert 'button[data-tool=\\\"leader\\\"].active' in s; assert 'button[data-tool=\\\"text\\\"].active' in s; assert 'button[data-tool=\\\"line\\\"].active' in s; m=re.search(r'<script>([\s\S]*?)</script>',s); assert m, 'script not found'; Path(r'build\kb911_check.js').write_text(m.group(1),encoding='utf-8')"
if errorlevel 1 exit /b 1
node --check build\kb911_check.js
if errorlevel 1 exit /b 1
python -c "from pathlib import Path; s=Path(r'src\main.cpp').read_text(encoding='utf-8'); assert 'KB911_V31_NATIVE_WINDOW_GUARD' in s; assert 'const bool samePaper' in s"
if errorlevel 1 exit /b 1
